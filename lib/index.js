...
console.log('using patched pouchdb 23.02.2022');
...
function openDatabase(openDatabases, api, opts, resolve, reject) {
  ...
  openReq.onsuccess = function (e) {
    ...
    idb.onabort = function (e) {
      ...
      idb.close();
      if (window.handleGlobalDBFailure) {
        window.handleGlobalDBFailure('global db failure: ' + e.target.error);
      }
    };
...
function IdbPouch(dbOpts, callback) {
  ...
  var waitForForeground = function() {
    if (window.waitForForegroundPromise) {
      console.debug('wait for foreground');
      return window.waitForForegroundPromise.then(function() {
        console.debug('in foreground!');
      });
    } else {
  ...
  var $ = function (fun) {
    return function () {
      ...
      waitForForeground().then(function() {
        return setup(openDatabases, api, dbOpts)
      }).then(function (res) {
